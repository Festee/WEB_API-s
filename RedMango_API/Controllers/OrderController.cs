...
namespace RedMango_API.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse>> DeleteOrder(int id)
        {
            try
            {
                var orderFromDb = _db.OrderHeaders.Include(u => u.OrderDetails)
                ...
                // Fshirja e detajeve të porosisë (nëse ka)
                if (orderFromDb.OrderDetails != null && orderFromDb.OrderDetails.Any())
                {
                    _db.OrderDetails.RemoveRange(orderFromDb.OrderDetails);
                }

                // Fshirja e porosisë
                _db.OrderHeaders.Remove(orderFromDb);
                _db.SaveChanges();
