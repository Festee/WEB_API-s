    ...
    if (result.error) {
      ...
      data.cartItems?.forEach((item: cartItemModel) => {
        ...
        const response: apiResponse = await createOrder({
          ...
          status: result.paymentIntent && result.paymentIntent.status === "succeeded" ? SD_Status.CONFIRMED : SD_Status.PENDING,
        });
  
      if (response) {
        if (response && response.data && response.data.result && response.data.result.status === SD_Status.CONFIRMED) {
